...
## 8. Group Lasso & Engineering Overhaul (August 2026)

**Changes:**
- `src/workflows/gl_rf_hybrid.py`: Implemented internal 80/20 train/validation split within `_gl_feature_selection` to run grid search on `group_reg` and `l1_reg`. Added `tqdm` progress tracking for Monte Carlo iterations.
- `src/workflows/group_lasso_workflow_config.py`: Added grid variables (`group_reg_grid`, `l1_reg_grid`) and `enable_hyperparam_search` flag.
- `src/workflows/group_lasso_workflow.py`: Fixed `LogisticGroupLasso` instantiation to properly utilize `warm_start` and `subsampling_scheme` defined in config, drastically reducing training time.
- `scripts/experiments/run_hybrid_all_datasets.py`: Created an orchestrator script that runs the pipeline on all 7 datasets and saves them in a unified timestamped `run_YYYY_MM_DD...` folder.
- Reorganized `output/` directory into categorized subdirectories (`gsm_runs/`, `gl_runs/`, `logs/`, `hybrid_runs/`, `figures/`, `misc/`).
...
| `src/workflows/group_lasso_workflow_config.py` | Added Grid Search parameters |
| `src/workflows/group_lasso_workflow.py` | Fixed `warm_start` injection |
| `src/workflows/gl_rf_hybrid.py` | Grid search implementation + `tqdm` loop |
...
| `scripts/experiments/run_hybrid_all_datasets.py`| **NEW** — Multi-dataset orchestrator |
